...
2. The page calls `GET /api/v1/telemetry` every 12 seconds — returns current metrics for all 26 nodes
3. Each node card shows CPU/Memory/Power bars
4. **Click any node card** → the UI calls `POST /api/v1/analytics/anomaly/detect/{node_id}` with that node's live metrics
...
3. The UI:
   ...
   - Fires 26 parallel `POST /api/v1/analytics/anomaly/detect/{node_id}` calls
   - Renders result cards sorted: **CRITICAL** first → **WARNING** → **NORMAL**
4. Each card shows:
   - `IF Score` — how far outside the normal distribution
   - `RF Confidence` — probability it is a genuine fault
...
1. Go to **Simulate** tab → select **Scenario 4: Compute Stress Peak**
2. Set: `cpu_percent=96`, `memory_percent=96`, `power_watts=1450`
3. Click **Run Simulation**
4. Result shows `FAIL ❌` with violations and LLM-enhanced recommendations
